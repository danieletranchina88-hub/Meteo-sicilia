...
prova('il fulmine e\' visto dall\'alto, non di lato', () => {
  ...
  assert.match(ritaglioMinimo(), /puntiFilamento\(/,
...
prova('il canale si assottiglia e si spegne verso la punta', () => {
  ...
  const usi = (ritaglioMinimo().match(/strokeSfumato\(/g) || []).length;
  assert.ok(usi >= 1, 'il tracciatore sfumato viene usato solo ' + usi + ' volte');
});
...
prova('mentre il lampo brilla il simbolo non lo copre', () => {
  ...
  const disegno = html.match(/function drawLiveStrikes\(\)[\s\S]*?\n {6}\}/);
  assert.match(disegno[0], /if \(s\.cloudIlluminated && eta < s\.durata \* 0\.8\) continue;/,
...
prova('la luce usa solo la trama del satellite corrente', () => {
  const mask = implementazione('costruisciMascheraNube');
  assert.match(html, /costruisciMascheraNube\(canvas, box, product\)/);
  assert.match(mask, /px\[i \+ 3\] = Math.round\(255 \* cloud\)/);
  assert.match(ritaglioMinimo(), /globalCompositeOperation = "destination-in"/);
  assert.doesNotMatch(ritaglioMinimo(), /tondo\(/, 'nessun disco di ripiego nel cielo sereno');
  assert.match(implementazione('pubblicaFotogrammaInCache'), /mascheraNube = frame.texture \|\| null/);
  assert.match(implementazione('publishSatelliteClouds'), /mascheraNube = texture/);
  assert.match(implementazione('publishSatelliteClouds'), /publicationToken !== cloudToken/);
  assert.match(implementazione('scaricaFotogramma'), /rememberCloudFrame\(key, url, box, size, texture\)/);
  assert.match(implementazione('drawLiveStrikes'), /s.cloudLight = null/);
  assert.match(implementazione('drawLiveStrikes'), /if \(!showSatelliteClouds && !strikeReducedMotion\(\)\) disegnaNuclei/);
  assert.match(html, /#strike-light-canvas \{ z-index: 5; mix-blend-mode: screen; \}/);
  assert.match(implementazione('clearVectors'), /strikeLightContext.clearRect/);
});

prova('la maschera conserva trasparenza, pieghe scure e bordi chiari', () => {
  const input = new Uint8ClampedArray(400);
  for (let p = 0; p < 100; p++) {
    const l = p < 50 ? 10 : p < 75 ? 120 : 240;
    input.set([l,l,l,255], p*4);
  }
  input.set([255,255,255,0], 396);
  input.set([255,180,0,255], 392);
  let output;
  const document = { createElement: () => ({
    getContext: () => ({ drawImage() {},
      getImageData: () => ({data: new Uint8ClampedArray(input)}),
      putImageData(image) { output = image.data; } })
  }) };
  const build = new Function('document','clamp', 'const MASCHERA_LATO=768;\n'
    + implementazione('costruisciMascheraNube') + '\nreturn costruisciMascheraNube;')(
      document, (v,a,b) => Math.max(a,Math.min(b,v)));
  const box = {west:1,east:2,south:3,north:4};
  const mask = build({width:10,height:10},box,{mode:'scene',layer:'rgb'});
  assert.equal(mask.layer,'rgb');
  assert.equal(output[3],0);
  assert.equal(output[399],0, 'pixel trasparenti non devono illuminarsi');
  assert.equal(output[395],0, 'luce urbana satura non deve diventare nube');
  assert.ok(output[303] > output[203] && output[203] > 0);
  assert.equal(build({width:10,height:10},box,{mode:'diagnostic'}),null);
  input.fill(240);
  assert.equal(build({width:10,height:10},box,{mode:'grey'}),null, 'quadro uniforme: nessuna nube inventata');
});

prova('la proiezione segue la rotazione e rifiuta viste non supportate', () => {
  const mask = {west:0,east:10,north:10,south:0,layer:'rgb'};
  let pitch=0;
  const make = (sat,history,product) => new Function('map','showSatelliteClouds',
    'cloudTimeSelected','cloudProduct','CLOUD_PRODUCTS','projectParticle',
    implementazione('proiezioneNube')+'\nreturn proiezioneNube;')(
      {getPitch:()=>pitch},sat,history,product,{rgb:{layer:'rgb'},ir:{layer:'ir'}},
      (x,y)=>({x:x-y,y:x+y}));
  assert.deepEqual(make(true,null,'rgb')(mask), {
    nw:{x:-10,y:10},ne:{x:0,y:20},sw:{x:0,y:0}});
  assert.equal(make(false,null,'rgb')(mask),null);
  assert.equal(make(true,123,'rgb')(mask),null);
  assert.equal(make(true,null,'ir')(mask),null);
  pitch=20;
  assert.equal(make(true,null,'rgb')(mask),null);
  assert.equal(make(true,null,'rgb')(null),null);
});

prova('la luce visibile e bianco freddo poco saturo, non uno spettro inventato', () => {
  const colours = [...ritaglioMinimo().matchAll(/rgba\((\d+),(\d+),(\d+),/g)]
    .map(m=>[+m[1],+m[2],+m[3]]);
  assert.ok(colours.length >= 5);
  for (const c of colours) {
    assert.ok(Math.min(...c)>=230);
    assert.ok(Math.max(...c)-Math.min(...c)<=25);
  }
  assert.match(ritaglioMinimo(), /"240,247,255"/);
  assert.doesNotMatch(implementazione('drawLiveStrikes'), /luceBrace\(/,
    'nessun raffreddamento arancione inventato sul satellite');
});

prova('luce finita, geografica e senza onda in espansione', () => {
  const draw = implementazione('drawLiveStrikes');
  assert.match(draw, /const km = 18 \+ 12 \* s.seme/);
  assert.match(draw, /const largo = Math.min\(148, Math.max\(2, Math.hypot/);
  assert.doesNotMatch(draw, /const largo = .*nube/);
  assert.match(ritaglioMinimo(), /globalCompositeOperation = "screen"/);
  assert.match(ritaglioMinimo(), /Math.min\(0.9, nube \* 0.9\)/);
  assert.match(ritaglioMinimo(), /"blur\(5px\)"/);
  const stops = [...ritaglioMinimo().matchAll(
    /light.addColorStop\(([0-9.]+), "rgba\(\d+,\d+,\d+,([0-9.]+)\)"/g)];
  assert.ok(stops.length>=5);
  assert.ok(stops.filter(m=>+m[1]>=0.6).every(m=>+m[2]<=0.12));
  const event = scarica(0.44);
  assert.ok(modulo.luceNube(event,event.ultimoColpo+250)<0.12);
  assert.ok(STRIKE_BAGLIORE_ATTESO<=420);
});

prova('movimento ridotto: niente lampi animati ma segni conservati', () => {
  const make = new Function('window',implementazione('strikeReducedMotion')
    + '\nreturn strikeReducedMotion;');
  assert.equal(make({matchMedia:()=>({matches:true})})(),true);
  assert.equal(make({})(),false);
  assert.match(implementazione('drawLiveStrikes'),/if \(strikeReducedMotion\(\)\) continue/);
  assert.match(implementazione('animateStrikes'),/!strikeReducedMotion\(\)/);
});
...
prova('il tick stereo resta un segnale breve, senza tuono simulato', () => {
  const tick = implementazione('suonaTick');
  assert.match(tick, /if \(cloudTimeSelected\) return/);
  assert.match(tick, /createStereoPanner/);
  assert.match(tick, /map.project\(\[lon, lat\]\)/);
  assert.match(tick, /-0.65, 0.65/);
  assert.match(tick, /1450 \* clamp\(tono, 0.92, 1.08\)/);
  assert.match(tick, /banda.Q.value = 0.65/);
  assert.match(tick, /node.disconnect\(\)/);
  assert.match(implementazione('proiezioneNube'), /!map.isSourceLoaded\("satellite-clouds"\)/);
});
