...
def process_data():
    ...
    # L'ambiente ICON-2I per le nubi in volume (base, gradiente, CAPE):
    # la texture si cuoce poi ogni mezz'ora sull'ultimo fotogramma EUMETSAT.
    ...
    for idx, filename in enumerate(file_list):
        ...
        for i in steps:
            ...
            try:
                ...
                # Nubi in volume: il modello non decide dove sono le nubi, ne'
    ...
    if catalog:
        ...
        if cloud_environment is not None and cloud_environment.hours:
            try:
                cloud_environment.save(os.path.join(TEMP_DIR, "cloud_environment.npz"))
                print(
                    f"   Ambiente nubi in volume: {len(cloud_environment.hours)} scadenze.",
                    flush=True,
                )
            except Exception as cloud_env_error:
                print(f"   Ambiente nubi in volume non salvato: {cloud_env_error}", flush=True)
