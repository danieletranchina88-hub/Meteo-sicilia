...
def calculate_rh_numpy(temp_k, dew_k):
    ...
    # Gestione sicura divisione per zero
...
def extract_raw_grid(ds, mask, var_names):
    """Estrae i valori crudi (numpy) ignorando le coordinate temporali per evitare crash"""
    try:
        # Cerca la prima variabile presente tra quelle indicate
        var_key = next((k for k in var_names if k in ds), None)
        if not var_key: return None
        
        # Ritaglia e restituisce i valori
        d_masked = ds[var_key].where(mask, drop=True)
        return d_masked.values # Restituisce np.array, non xarray DataArray
    except:
        return None
...
def process_data():
    ...
    for idx, filename in enumerate(file_list):
        ...
        try:
            ...
            ds_thermo = None
            try: ds_thermo = xr.open_dataset(TEMP_FILE, engine='cfgrib', 
                backend_kwargs={'filter_by_keys': {'typeOfLevel': 'heightAboveGround', 'level': 2}})
            except: pass

            # 3. Pressione - Level meanSea (Trovato dal LOG: variabile 'pmsl')
            ds_press = None
            try: ds_press = xr.open_dataset(TEMP_FILE, engine='cfgrib', 
                backend_kwargs={'filter_by_keys': {'typeOfLevel': 'meanSea'}})
            except: pass

            # 4. Pioggia - Level surface (Accumulo)
            ds_rain = None
            try: ds_rain = xr.open_dataset(TEMP_FILE, engine='cfgrib', 
        ...
        for i in steps:
            try:
                ...
                # --- MASCHERA GEOGRAFICA ---
                # Ordiniamo e creiamo la maschera usando il dataset del Vento
                ...
                # Se la maschera è vuota, salta
                if cut_w.latitude.size == 0: continue

                # --- ESTRAZIONE DATI (NUMPY PURO) ---
                ...
                # --- ALTRI VARIABILI (Sincronizzate via maschera) ---
                ...
                if ds_thermo:
                    ...
                    # Estraiamo i Numpy array crudi
                    t_raw = extract_raw_grid(dt_step, mask, ['t2m', 't']) # Temp Kelvin
                    d_raw = extract_raw_grid(dt_step, mask, ['d2m', '2d']) # DewPoint Kelvin
                ...
                # Fallback pressione 1013 solo se tutto è 0
                ...
                step_data = {
                    ...
                    "press": np.round(press,1).flatten().tolist(), # Ora reale hPa
                    "rh": np.round(rh_val,0).flatten().tolist()    # Ora reale %
                }
