...
def compute_downsample_factor(ny, nx, max_pixels=MAX_PIXELS):
    """Ritorna un fattore intero >=1 per tenere ny*nx <= max_pixels."""
        ...
        for i in steps:
            try:
                ...
                if f > 1:
                    ...
                    v_val = downsample_2d(v_val, f)
                    lat = downsample_1d(lat, f)
                ...
                step_data = {
                    ...
                    "wind_u": {
                        ...
                        "data": np.round(u_val, 1).flatten().tolist()
                    },
                    "wind_v": {
                        "header": {**header, "parameterCategory": 2, "parameterNumber": 3},
                        "data": np.round(v_val, 1).flatten().tolist()
                    },
                ...
                if not any(x['hour'] == step_hours for x in catalog):
                    catalog.append({
                        "file": out_name,
                        "label": f"{valid_dt.strftime('%d/%m %H:00')}",
                        "hour": step_hours
                    })
